...
contains
  ...
  subroutine runge_kutta
    ...
    ! OR                                                                        !
    ! output5.dat : data file containing x, V(x)                                !
    !===========================================================================!
    
    
    !===============================DECLARATIONS==================================!
    integer                        :: i,n,ierr
    real(kind=dp),save             :: x,h,start,finish,wave_n, v_temp
    ...
    !Open the file ready to start writing output data for option 5
    if (output_option == 5) then
      open(35,file='output5.dat',status='replace',iostat=ierr)
      if (ierr/=0) stop "Error in opening output5.dat"
    end if
    ...
    do i=1,n
      ...
      !For option 5 write the data for x and V(x) to a file
      if (output_option == 5) then
        v_temp = v(x)
        write(35,*) x, v_temp
      end if
    ...
    !Close output5.dat
    if (output_option == 5) then
      close(35,iostat=ierr)
      if (ierr/=0) stop "Error in closing output5.dat"
    end if
...
program ode_solver
  ...
  ! things depending on the option selected.                                    !
  !=============================================================================!
  ...
  !For options 1,2 and 5 ask if a value of T or R is also required 
  !and if so calls the find_coeff subroutine to calculate it
  if (output_option == 1 .or. output_option == 2 .or. output_option == 5) then
    print*, 'Do you also want a value of Transmission Coefficient? (Type Yes or No)'
    read (*,*) answer
