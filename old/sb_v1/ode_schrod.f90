...
CONTAINS
  ...
  subroutine read_pot
    IMPLICIT NONE 
    !===========================================================================!
    ! Reads the form and dimensions of the given potential from a file.         !
    ! Then depending on the option selected, reads the energy parameters.       !
    !---------------------------------------------------------------------------!
    ! Input:                                                                    !
    ! input2.dat                                                                !
    !---------------------------------------------------------------------------!
    ! Output:                                                                   !
    ! output_option, pot_form, height, a, b, c, d, para_cons                    !
    ! E, E_start, E_finish, E_n                                                 !
    !===========================================================================!
    
    !Open the second input file to read the parameters in
    open(10,file='input2.dat',status='old',iostat=ierr)
    if (ierr/=0) stop "Error in opening input.dat"

    !Read the output option and make sure its an allowed option
    ...
    !If option 3 or 4 is selected, read the start and finish energies
