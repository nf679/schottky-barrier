...
program ode_solver
  ...
  !===============================DECLARATIONS==================================!
  integer :: i, norm_n
  real(kind=dp) :: e_h, num_T, norm, norm_h, E_temp
  ...
  !find the normalisation constant for the gaussian
  norm = 0.0_dp
  norm_n = 1000
  norm_h = (e_finish - e_start) / real(norm_n)
  E_temp = e_start
  do i=1, norm_n
    norm = norm + gauss(E_temp)
    E_temp = E_temp + norm_h
  end do
  norm = norm * norm_h
  print*, norm
  ...
  open(40, file='gaussian.dat', status='replace', iostat=ierr)
  if (ierr/=0) stop "Error in opening gaussian.dat"
  open(45,file='intensity.dat', status='replace',iostat=ierr)
  if (ierr/=0) stop "Error in opening intensity.dat"
  ...
  do i=0,(e_it-1)
    ...
    !Write the Gaussian data, scaling the gaussian
    write(40,*) E, gauss(E) * norm

    !Write the data for the intensity of particles transmitted
    write(45,*) E, (1.0_dp-refl)*real(num_e)*gauss(E)*norm
  ...
  close(40,iostat=ierr)
  if (ierr/=0) stop "Error closing gaussian.dat"
  close(45,iostat=ierr)
  if (ierr/=0) stop "Error closing intensity.dat"
