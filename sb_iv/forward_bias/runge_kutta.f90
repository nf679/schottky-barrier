...
CONTAINS
  ...
  SUBROUTINE runge_kutta
    ...
    ! potential.dat (uncomment)  : data file containing x, V(x)                 !
    !===========================================================================!
...
PROGRAM ode_solver
  ...
  !Open a file to write the bias data to
  OPEN(55,file='bias.dat',status='replace',iostat=ierr)
  IF (ierr/=0) STOP "Error in opening bias.dat"

  !Open a file to write the i-v characteristics to
  OPEN(60, file='current.dat', status='replace', iostat=ierr)
  IF (ierr/=0) STOP "Error in opening current.dat"

  !Set the initial energy of the electron to account for the potential
  E = E + bias
  ...
  DO i=1,bias_n
    ...
    !Write the bias data to the file
  ...
  CLOSE(60,iostat=ierr)
  IF (ierr/=0) STOP "Error in closing current.dat"
